...
const {VillageReturneditems,VillageSolditems,VillageAllitems} = db
...
export const confirmReturnedItem = async (req, res) => {
    try {
        ...
        const returnedItem = await VillageReturneditems.findByPk(id);
        if (!returnedItem) {
        ...
        const item = await VillageAllitems.findByPk(returnedItem.sold_item_id);
