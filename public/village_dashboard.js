fetch('/dashboarddata/todaycash')
  ...
  fetch('/village_dashboarddata/sales')
  .then(response => response.json())
  .then(function (data) {
    const dataElement = document.getElementById('village_dataDisplayy');
    dataElement.innerText = data.totalInstock;
  })
  .catch(error => console.error(error));

fetch('/village_dashboarddata/items')
  .then(response => response.json())
  .then(function (itemData) {
    const allItemsElement = document.getElementById('villageallitemsDisplay')
    allItemsElement.innerText = itemData.totalItems;
  })
  .catch(error => console.error(error));

  fetch('/village_dashboarddata/todayitems')
  .then(response => response.json())
  .then(function (itemData) {
    const allItemsElement = document.getElementById('villageitemsDisplay')
    allItemsElement.innerText = itemData.totalItems;
  })
  .catch(error => console.error(error));

  
fetch('/village_dashboarddata/todayprofit')
.then(response => response.json())
.then(function (itemData) {
...
fetch('/village_dashboarddata/instock')
.then(response => response.json())
.then(function (itemData) {
...
fetch('/village_dashboarddata/sold')
  .then(response => {
    if (!response.ok) {
...
fetch('/partials/_topbar.html')
  ...
  function renderTable(data) {
    const salesTableBody = document.getElementById('villagereportItemsTable');
  ...
  function showvillageViewModal() { 
    const modal = document.getElementById('villageviewModal');
  ...
  document.addEventListener('DOMContentLoaded',async () => {
    ...
    const form = document.getElementById('villageallitemsModalSubmit');
    if (form) {
        form.addEventListener('submit', async (event) => {
            ...
            try {
                const response = await fetch('/village_allitemsroute/itemscreate', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
...
async function fetchTransactionTotals() {
  try {
    const response = await fetch('/village_dashboarddata/mpesa');
    ...
    // Update the DOM
    document.getElementById('village_cash').textContent = `ksh${cash}`;
    document.getElementById('village_mpesa').textContent = `ksh${mpesa}`;
...
function updateTransactionChart(mpesa, cash) {
  const ctx = document.getElementById('village_transaction-history').getContext('2d');
  new Chart(ctx, {
...
document.addEventListener('DOMContentLoaded', checkPermissions);
  ...
  function returnItem(event, id) {
    ...
    fetch(`/village_dashboarddata/returnitem/${id}`)
      .then(response => response.json())
      .then(returnDetails => {
...
function updatereturnItem(event) {
  ...
  fetch(`/village_dashboarddata/return/${id}`, {
      method: 'PUT',
      headers: {
          'Content-Type': 'application/json'
      },
