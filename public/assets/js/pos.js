...
let storage = {
  get: async (key) => await ipcRenderer.invoke('get-setting', key),
  set: async (key, value) => await ipcRenderer.invoke('set-setting', key, value),
};

let img_path = '';
const { ipcRenderer } = require('electron');

ipcRenderer.invoke('get-appdata-path').then(appDataPath => {
  img_path = appDataPath + '/POS/uploads/';
  console.log("✅ Image path is", img_path);

  // You can now safely use img_path in your logic
});
